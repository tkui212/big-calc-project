...
import Dchain from "../objects/chain"
import $ from "jquery";
...
export class Cir extends Body {
  constructor(props) {
    ...
    this.DC.locked.push(this.parent.DC)
    this.parent.DC.locked.push(this.DC)
    this.radius = props.radius;
    this.data=new Data({x: props.x, y:props.y, id:this.id+"D"});
    this.conner=this.data
    this.DataHolder =
    define(this, `x`, () => {
      return this.conner.x;
    }, num => {
      this.conner.x = num;
    });
    define(this, `y`, () => {
      return this.conner.y;
    }, num => {
      this.conner.y = num;
    });
  }
  ...
  connect(to){
    this.disconnect()
    this.DC.connect(to.DC);
    to.conner.cons.push(this)
    this.elem.style.cx = `calc(${this.conner.cx}*1px)`;
    this.elem.style.cy = `calc(${this.conner.cy}*1px)`;
  }
  disconnect(){
    this.DC.disconnect();
    this.conner.removeCon(this.id)
    this.data.x=this.conner.x
    this.data.y=this.conner.y
    this.conner=this.data
    this.elem.style.cx = `calc(${this.conner.cx}*1px)`;
    this.elem.style.cy = `calc(${this.conner.cy}*1px)`;
  }
  toConsole(copy) {
    let obj = {
      id: this.id + "console",
      text: this.id + " data",
      values: { x: this, y: this, id: this },
      parent: this,
      left: parseInt(this.x) + 50,
      top: parseInt(this.y) + 50,
      dataSource: this
    };
...
export class Line extends Body {
  constructor(props) {
    ...
    define(this, "path", () => {
      ...
      document
        .getElementById("all")
        .style.setProperty(`--${this.id}-path`, `${num}`);
    });
    ...
    if (typeof props.x == "number" && typeof props.y == "number") {
      this.point2.x = exactMath.formula(
        `${this.x1}+${Math.sin(toRadians(this.angle))}*${this.F}`
      );
      this.point2.y = exactMath.formula(
        `${this.y1}-${Math.cos(toRadians(this.angle))}*${this.F}`
      );
    ...
    console.log(
      evaluate(`((${this.x1}-${this.x2})^2+(${this.y1}-${this.y2})^2)^0.5`)
    );
  }
      ...
      "elems here": () => {
        console.log(
          document.elementsFromPoint(
            parseInt(window.mouseX),
            parseInt(window.mouseY)
          )
        );
      }
    };
  ...
  render() {
    return (
      <circle
        ...
        fill={"white"}
        className={"point"}
        style={{ cx: `${this.conner.cx}`, cy: `${this.conner.cy}` }}
        key={this.id}
      />
    );
  }
...
export class ForceC extends Line {
  constructor(props) {
    ...
    define(this, "angle2", () => {
      ...
      document
        .getElementById("all")
        .style.setProperty(`--${this.id}-deg`, `rotate(${num}deg)`);
  ...
  render() {
    ...
    return [
      <img
        ...
        className={"line"}
        style={{
          top: `calc(${this.data.cy}*1px)`,
          left: `calc(${this.data.cx}*1px)`,
          width: `20px`,
          position: ` absolute`,
          height: ` ${this.F}px`,
          zIndex: ` 99`,
          transformOrigin: ` top`,
          transform: `var(--${this.id}-deg)`,
